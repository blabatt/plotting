...
\textbf{replot} \textit{ and }\textbf{refresh}\textit{ repeat the last }\textbf{plot}\textit{ command. }\textbf{with}\textit{ sets \say{plot style}-specific characteristics.} \\[2mm]
\textit{Plot examples:}\\
\entry{40mm}{plot sin(x), cos(x)}{multiple PEs}\\
\entry{40mm}{plot for \dots}{for clause}\\
\entry{40mm}{plot \textquotedbl file\textquotedbl with lines,}{with clause}\\
\entry{40mm}{\phantom{xxx}\textquotedbl file2\textquotedbl with points}{}\\
\entry{40mm}{plot [-pi:pi*2] tan(x)}{ranges}\\
\entry{40mm}{plot [t=1:10] [-pi:pi*2] tan(t)}{\href{http://www.gnuplot.info/demo/param.html}{parametric}}\\
...
\textbf{using}\textit{ phrase maps }<data>\textit{ to the abscissa \& ordinate (or angle and radius) as well as \say{plot style}-specific characteristics. Using examples:}\\
