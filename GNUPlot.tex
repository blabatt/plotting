\section{GNUPlot}
\textit{General script pattern involves an arbitrary number of }\textbf{set}\textit{ commands for setup of axes, titles, fonts, and other styles; followed by one or more }\textbf{plot}\textit{ commands, each of which renders output to the default or designated \say{terminal}. See \href{here}{http://gnuplot.info/demos/} for numerous demos.}
...
\textit{There are a number of variations on a theme, eg to create a \say{bee swarm} plot, use }\textbf{with points}\textit{ and }\textbf{set jitter}.
